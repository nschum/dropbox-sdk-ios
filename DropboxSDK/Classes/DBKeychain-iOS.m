...
+ (NSDictionary *)credentials {
	...
	if (status == noErr && foundValue) {
		...
		if (status != errSecItemNotFound) {
			DBLogWarning(@"DropboxSDK: error reading stored credentails (%ld)", status);
		}
		return nil;
	}
}
...
+ (void)setCredentials:(NSDictionary *)credentials {
	...
	if (status != noErr) {
		DBLogWarning(@"DropboxSDK: error saving credentails (%ld)", status);
	}
}
...
+ (void)deleteCredentials {
	...
	if (status != noErr) {
		DBLogWarning(@"DropboxSDK: error deleting credentials (%ld)", status);
	}
}
